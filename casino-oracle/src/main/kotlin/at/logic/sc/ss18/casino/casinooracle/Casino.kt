...
class Casino(contractAddress: String,
    ...
    fun observeBetPlacedEvents(filter: EthFilter): Observable<BetPlacedEvent> =
            web3j.ethLogObservable(filter).map { log ->
                val eventValues = extractEventParametersWithLog(BET_PLACED_EVENT, log)
                BetPlacedEvent(
                        log = log,
                        betTimestamp = eventValues.nonIndexedValues[1].value as BigInteger,
                        betPlacedTimestamp = eventValues.nonIndexedValues[2].value as BigInteger,
                        rise = eventValues.nonIndexedValues[3].value as Boolean
                )
            }
